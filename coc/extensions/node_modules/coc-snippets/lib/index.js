...
async function activate(context) {
  ...
  if (configuration.get("autoTrigger", true)) {
    ...
    import_coc8.events.on("TextInsert", async (bufnr, info) => {
      ...
      if (edits.length > 1) {
        ...
        import_coc8.window.showMessage("Multiple snippet found for auto trigger, check output by :CocCommand workspace.showOutput", "warning");
      }
      if (inserting)
        return;
      inserting = true;
      await import_coc8.commands.executeCommand("editor.action.insertSnippet", edits[0]);
      inserting = false;
    }, null, subscriptions);
  }
  ...
  async function doExpand(bufnr) {
    ...
    if (edits.length == 1) {
      ...
      if (idx == -1)
        return;
      await import_coc8.commands.executeCommand("editor.action.insertSnippet", edits[idx]);
    }
    return true;
  }
  ...
  subscriptions.push(import_coc8.commands.registerCommand("snippets.editSnippets", async (text) => {
    ...
    if (text) {
      ...
      await import_coc8.commands.executeCommand("editor.action.insertSnippet", edit);
    }
  }));
